...
>
  ...
  {#if highlightForMoveSuggestion}
    {#if !!pieceNumber}
      <div class="w-11/12 border-background/50 border-[6px] rounded-full transition-all duration-75 group-hover:bg-background/50 group-hover:border-transparent group-hover:w-full group-hover:rounded-none aspect-square absolute-center"></div>
    {:else}
      <div class="w-1/3 bg-background/50 transition-all duration-75 group-hover:w-full group-hover:rounded-none rounded-full aspect-square absolute-center"></div>
    {/if}
  {:else if highlightLastMove}
    <div class="absolute inset-0 bg-accent/60">
  ...
  {#if highlightSelectedTile}
    ...
    <div class="absolute inset-0 bg-red-700 text-red-700">
    </div>
  {/if}
