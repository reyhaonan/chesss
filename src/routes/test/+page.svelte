<script lang="ts">
	...
	const depthExpectation = [
		400, 8_092, 197_281, 4_865_609, 119_060_324, 3_195_901_860, 84_998_978_956
	];

	let counter = [0, 0, 0, 0];

	let [boardArray, turn, castlingRights, enPassantTarget, halfMoveClock, fullMoveClock] =
...
<div class="text-neutral-50 text-2xl">
	...
	<input
		...
		max="8"
		bind:value={DEPTH}
		class="bg-transparent rounded-md border border=neutral-50 mt-32 p-4"
	/>
	<button
		disabled={typeof DEPTH !== 'number' || DEPTH < 1 || DEPTH > 8}
		on:click={performTest}
		class="p-4 bg-slate-50 disabled:bg-slate-600 rounded-md mt-8 text-slate-900"
		>Perform Test with {DEPTH ?? 'Invalid'} depth</button
	>
</div>
