...
export const convertFENToBoardArray = (FEN: string): BoardInfoArray => {
	...
	let [boardInfo, turn, castlingRights, enPassantTarget, halfMoveClock, fullMoveClock] =
		FEN.split(' ');
	...
	// En Passant
	newEnPassantTarget =
		enPassantTarget === '-' ? null : convertAlgebraicNotationToNumber(enPassantTarget);
...
export const generateMoves = (
	...
	if (futureCheck > 0)
		tempMoveList = tempMoveList.filter((move) => {
			let {
				newBoardArray,
				newEnPassantTarget,
				newTurn,
			...
			return (
				...
				!generateMoves(
					...
					(move) =>
						Piece.isType(newBoardArray.get(move.target), PieceType.King) &&
						Piece.sameColor(newBoardArray.get(move.target), currentTurn)
				)
			);
		});
